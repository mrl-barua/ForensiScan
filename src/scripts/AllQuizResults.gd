...
func determine_quiz_type(quiz_name: String) -> String:
	"""Determine if a quiz is prelim or midterm based on its name"""
	if "prelim" in quiz_name.to_lower() or "Prelim" in quiz_name:
		return "prelim"
	elif "midterm" in quiz_name.to_lower() or "Midterm" in quiz_name:
		return "midterm"
	else:
		return "unknown"
...
func clear_results_lists():
	"""Clear all existing result cards from both lists"""
...
func animate_result_cards():
	...
	# Collect all cards from both lists
