...
@export var header_text: String = "PLAIN IMPRESSIONS:"
@export var detail_one_text: String = "-are made on the card by simply pressing the four fingers on it at a slight angle. They should be showing the tips to one-quarter inch below the first joint. Thumbs are then printed by simply re-inking and pressing them on the block next to the plain finger impressions. The purpose of plain impressions is to verify the order of the rolled impressions and to show certain characteristics that are sometimes distorted in the rolled prints."
...
func _ready():
	...
	ProgressManager.update_lesson_progress("midterm", 4)
