...
@export var detail_one_text: String = "- Deformed fingers or hands. If the hands and fingers are so deformed that the regular printing procedure cannot be followed, prints can be made by applying the ink directly to the fingers with a spatula or small roller. A square, self-sticking replacement tab can be rotated around the finger. When a satisfactory print has been made, this square is then affixed to the appropriate box of the fingerprint card.
"
@export var detail_two_text: String = "- In cases where there are extra fingers (usually little fingers or thumbs), the innermost five are printed on the card, and the extra digit is printed on the reverse of the card.
"
...
func _ready():
	...
	ProgressManager.update_lesson_progress("midterm", 6)
