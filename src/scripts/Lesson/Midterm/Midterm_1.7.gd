...
@export var detail_one_text: String = "Make a notation if a finger or fingertip is amputated (or has some other deformity) in the appropriate box on the card specifying which finger is deformed. (Example: 'AMP, LEFT INDEX, 1ST JOINT, FEB 1943' or 'LEFT INDEX TIP AMP' or 'BEST PRINTS POSSIBLE'). Ensure that the writing stays within the appropriate box.
"
@export var detail_two_text: String = "Excessive perspiration. Because of the subject's excessive perspiration, some inked impressions will be indistinct. Wipe each finger with a cloth and immediately ink and roll the finger on the fingerprint card. Follow this process with each finger. It is also possible to wipe the fingers with alcohol or similar fluid that would act as a drying agent.
"
...
func _ready():
	...
	ProgressManager.update_lesson_progress("midterm", 7)
