...
@export var detail_one_text: String = "DOUBLE LOOPS- the ridges in this pattern possess two well-defined loops which more or less embrace each other. The re are deltas, which are located below the loop formation and offset on either side.
"
...
func _ready():
	...
	ProgressManager.update_lesson_progress("midterm", 48)
