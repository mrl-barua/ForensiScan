...
@export var detail_one_text: String = "CENTRAL POCKET LOOP- the ridges in this particular pattern conform to the explanation given of the plain whorl with exception that when an imaginary line is drawn between the right and left delta, it will not touch the circuiting ridge."
@export var detail_two_text: String = "			- it consist of 2 deltas (left & right portion)"
...
func _ready():
	...
	ProgressManager.update_lesson_progress("midterm", 47)
