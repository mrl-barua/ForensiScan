...
func setup_simplified_content():
	...
	# Get quiz results from QuizManager
	var latest_score = QuizManager.get_latest_score()
	var score_info = ""
	
	if latest_score.has("score"):
		var score = latest_score.get("score", 0)
...
⏳ Redirecting to detailed results in 3 seconds...""" % score_info
...
func setup_auto_redirect():
	...
	await get_tree().create_timer(4.0).timeout  # Show content for 4 seconds
	
	print("🔄 Auto-redirecting to detailed results...")
	get_tree().change_scene_to_file("res://src/scenes/Quiz/Midterm/Midterm_Quiz_Result.tscn")
