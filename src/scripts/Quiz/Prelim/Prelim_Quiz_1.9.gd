...
func _on_MultipleChoice_answer_selected(option: String, letter: String):
	print("Answer selected Question 9 - Letter: ", letter, " Option: ", option)
	QuizManager.submit_answer(option)
	if QuizManager.next_question():
		get_tree().change_scene_to_file("res://src/scenes/Quiz/Prelim/Prelim_Quiz_1.10.tscn")

func _on_MultipleChoice_time_expired():
	print("Quiz: Time expired for question 9 - proceeding to next")
