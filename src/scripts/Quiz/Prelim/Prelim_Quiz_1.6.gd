...
func _on_MultipleChoice_answer_selected(option: String, letter: String):
	print("Answer selected Question 6 - Letter: ", letter, " Option: ", option)
	QuizManager.submit_answer(option)
	if QuizManager.next_question():
		get_tree().change_scene_to_file("res://src/scenes/Quiz/Prelim/Prelim_Quiz_1.7.tscn")

func _on_MultipleChoice_time_expired():
	print("Quiz: Time expired for question 6 - proceeding to next")
